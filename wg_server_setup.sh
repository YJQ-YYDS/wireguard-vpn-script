...
if check_server_config && grep -qE "PostUp.*-o ${INTERFACE}( |$)" /etc/wireguard/wg0.conf; then
    skip_step "创建 /etc/wireguard/wg0.conf (已存在且网卡匹配)"
elif check_server_config; then
    # 配置已存在但公网网卡变化：仅替换 PostUp/PostDown 行，保留已有 Peer，避免覆盖丢失客户端配置
    step_start "公网网卡变化，仅更新 wg0.conf 的 PostUp/PostDown (保留已有 Peer)"
    sed -i "s|^PostUp = .*|PostUp = iptables -A FORWARD -i wg0 -j ACCEPT; iptables -t nat -A POSTROUTING -o $INTERFACE -j MASQUERADE|" /etc/wireguard/wg0.conf
    sed -i "s|^PostDown = .*|PostDown = iptables -D FORWARD -i wg0 -j ACCEPT; iptables -t nat -D POSTROUTING -o $INTERFACE -j MASQUERADE|" /etc/wireguard/wg0.conf
    warn "配置已更新，需重启服务生效: systemctl restart wg-quick@wg0"
    step_end "更新 wg0.conf 网卡配置"
else
...
if check_ufw_ports && check_ufw_forward; then
    ...
    if command -v ufw &> /dev/null; then
        # 默认放行 22/tcp；若 sshd 实际监听非 22 端口（如 2202），一并放行，避免 ufw reload 后自锁
        SSH_PORT=$(grep -Ei '^[[:space:]]*Port[[:space:]]+[0-9]+' /etc/ssh/sshd_config 2>/dev/null | awk '{print $2}' | head -n1)
        [ -z "$SSH_PORT" ] && SSH_PORT=$(ss -tlnp 2>/dev/null | grep sshd | grep -oE ':[0-9]+[[:space:]]' | grep -oE '[0-9]+' | sort -u | head -n1)
        [ -z "$SSH_PORT" ] && SSH_PORT=22
        ufw allow 22/tcp > /dev/null 2>&1 || warn "添加 22/tcp 规则失败，可能已存在"
        if [ "$SSH_PORT" != "22" ]; then
            ufw allow ${SSH_PORT}/tcp > /dev/null 2>&1 || warn "添加 ${SSH_PORT}/tcp 规则失败，可能已存在"
        fi
        ...
        if [ "$SSH_PORT" != "22" ]; then
            info "UFW 已放行端口: 22/tcp, ${SSH_PORT}/tcp (sshd 实际端口), 51820/udp，并允许转发。"
        else
